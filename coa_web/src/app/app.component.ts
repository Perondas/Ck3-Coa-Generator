...
export class AppComponent implements OnInit {
  ...
  fileControl = new FormControl(
    [],
    [FileValidators.required, FileValidators.uniqueFileNames]
  );
  resolution = 100;
  isTitle = false;
  showControls = false;
  ...
  async onReducedColors() {
    ...
    coa_from_image_reduced_colors(array, this.isTitle, 20, this.resolution)
      .then((result) => {
        if (result) {
          this.result = result;
        } else {
          this.dialog.open(MessageBoxComponent, {
            data: {
              message: 'A error occurred. Make sure to submit a valid image!',
            },
          });
        }
      })
  ...
  async onAllColors() {
    ...
    let buffer = await file.arrayBuffer();
    let array = new Uint8Array(buffer);
    coa_from_image_all_colors(array, this.isTitle, this.resolution)
      .then((result) => {
        if (result) {
          this.result = result;
        } else {
  ...
  async onVanillaColors() {
    ...
    let buffer = await file.arrayBuffer();
    let array = new Uint8Array(buffer);
    coa_from_image_vanilla_colors(array, this.isTitle, this.resolution)
      .then((result) => {
        if (result) {
          this.result = result;
        } else {
